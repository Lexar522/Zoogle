...
<div class="catalog-toolbar">
    <div class="catalog-toolbar__head">
        <div class="catalog-toolbar__head-top">
            ...
            <div class="catalog-toolbar__chips" role="group" aria-label="Швидке сортування">
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'newest') is-active @endif" data-catalog-sort-preset="newest">Новіші</button>
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'title_asc') is-active @endif" data-catalog-sort-preset="title_asc">А–Я</button>
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'title_desc') is-active @endif" data-catalog-sort-preset="title_desc">Я–А</button>
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'price_asc') is-active @endif" data-catalog-sort-preset="price_asc">Ціна ↑</button>
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'price_desc') is-active @endif" data-catalog-sort-preset="price_desc">Ціна ↓</button>
                <button type="button" class="catalog-toolbar__chip @if ($sortVal === 'popular') is-active @endif" data-catalog-sort-preset="popular">Популярні</button>
            </div>
        </div>
    </div>
</div>
...
@if (($filters['q'] ?? '') !== '' || ($filters['category'] ?? 0) > 0 || ! empty($filters['on_sale'] ?? false))
    <div class="catalog-active-filters" style="display:flex;flex-wrap:wrap;gap:.45rem;">
        @if (($filters['q'] ?? '') !== '')
            <a
                class="btn secondary"
...
@if ($listings->hasMorePages())
    <div class="catalog-results-more" role="region" aria-label="Наступні товари в каталозі">
        <p class="catalog-results-more__hint">Є ще товари — перегорніть сторінку або додайте наступну порцію сюди.</p>
...
<div class="home-shop-panel catalog-listing-panel catalog-listing-panel--prompt">
    <p class="catalog-results__empty-hint-text">Оберіть категорію в меню вище або скористайтеся пошуком.</p>
</div>
@endif
